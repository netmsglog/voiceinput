...
1. Download `VoiceInput-v2.0.0.zip` from [Releases](https://github.com/netmsglog/voiceinput/releases)
   从 [Releases](https://github.com/netmsglog/voiceinput/releases) 下载 `VoiceInput-v2.0.0.zip`
...
4. **Stop speaking** → VAD detects silence and auto-stops ("Pop" sound) / **停止说话** → VAD 检测静默自动停止（听到"Pop"提示音）
...
3. **VAD auto-stop / 自动停止** — Silero VAD (ONNX, ~2MB) monitors audio in real-time. After detecting speech followed by 1.5s of silence, recording stops automatically.
